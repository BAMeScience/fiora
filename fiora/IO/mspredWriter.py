...
def write_labels(df, output_file, label_map, from_metabolite=True):
    ...
    try:
        df.rename(columns=label_map)[label_header].to_csv(
            output_file, index=False, sep='\t'
        )
    except Exception as _:
        raise NameError(
            f'Failed to write labels file. Make sure file path is correct. Make sure all headers are present in DataFrame {label_header}. Use label_map to rename columns.'
        )
...
def write_spec_files(df, directory, spec_tag='spec'):
    for i, row in df.iterrows():
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(
                '>parentmass '
                + str(metabolite.ExactMolWeight + ADDUCT_WEIGHTS[row['Precursor_type']])
                + ' \n'
            )
...
def write_dataset(
    ...
    write_labels(
        df,
        output_file=os.path.join(directory, 'labels.tsv'),
        label_map=label_map,
        from_metabolite=True,
    )
    write_labels(
        df.iloc[::-1],
        output_file=os.path.join(directory, 'reverse_labels.tsv'),
        label_map=label_map,
        from_metabolite=True,
    )
