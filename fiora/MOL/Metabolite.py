...
class Metabolite:
    def __init__(
        self, SMILES: str | None, InChI: str | None = None, id: int | None = None
    ) -> None:
        if SMILES:
            self.SMILES = SMILES
        ...
        self.morganFinger = AllChem.GetMorganFingerprintAsBitVect(
            self.MOL, 2, nBits=2048
        )  # 1024
        self.morganFinger3 = AllChem.GetMorganFingerprintAsBitVect(
            self.MOL, 3, nBits=2048
        )  # 1024
    ...
    def compute_graph_attributes(
        self,
        node_encoder: AtomFeatureEncoder | None = None,
        bond_encoder: BondFeatureEncoder | None = None,
        ...
        self.edge_forward_direction = torch.tensor(
            [[bool(u < v) for u, v in self.edges_as_tuples]], dtype=torch.bool
        ).t()
        self.edge_backward_direction = torch.tensor(
            [[bool(u > v) for u, v in self.edges_as_tuples]], dtype=torch.bool
        ).t()
    ...
    def add_metadata(
        self,
        metadata,
        covariate_encoder: CovariateFeatureEncoder = None,
        rt_feature_encoder: CovariateFeatureEncoder = None,
        ...
        if covariate_encoder:
            ...
            if 'ce_steps' in metadata:
                ...
                self.ce_steps = torch.tensor([np.nan] * 7, dtype=torch.float).unsqueeze(
                    0
                )
            ...
            self.setup_features_per_edge = torch.zeros(
                len(self.edges_as_tuples), 0, dtype=torch.float32
            )

        if rt_feature_encoder:
            self.rt_setup_features = rt_feature_encoder.encode(
    ...
    def fragment_MOL(self, depth=1):
        ...
        self.fragmentation_tree.build_fragmentation_tree(
            self.MOL, self.edges_as_tuples, depth=depth
        )
    ...
    def extract_subgraph_features_from_edges(self) -> None:
        if self.fragmentation_tree is None:
            self.subgraph_elem_comp = torch.zeros(
                0, 2 * len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN), dtype=torch.float32
            )
            self.subgraph_idx_left = torch.full(
                (0, MAX_SUBGRAPH_NODES), -1, dtype=torch.int64
            )
            self.subgraph_idx_right = torch.full(
                (0, MAX_SUBGRAPH_NODES), -1, dtype=torch.int64
            )
            return
        ...
        self.subgraph_elem_comp = torch.zeros(
            num_edges, 2 * len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN), dtype=torch.float32
        )
        self.subgraph_idx_left = torch.full(
            (num_edges, MAX_SUBGRAPH_NODES), -1, dtype=torch.int64
        )
        self.subgraph_idx_right = torch.full(
            (num_edges, MAX_SUBGRAPH_NODES), -1, dtype=torch.int64
        )
        ...
        for i, edge in enumerate(self.edges):
            ...
            edge_elem_comp = torch.zeros(
                2 * len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN), dtype=torch.float32
            )

            if left_fragment is not None and right_fragment is not None:
                # Compute element composition for left and right fragments
                edge_elem_comp[: len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN)] = (
                    CovariateFeatureEncoder.get_element_composition(
                        self.Graph.subgraph(left_fragment.subgraphs[0])
                    )
                    .clone()
                    .detach()
                    .to(torch.float32)
                ...
                edge_elem_comp[len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN) :] = (
                    CovariateFeatureEncoder.get_element_composition(
                        self.Graph.subgraph(right_fragment.subgraphs[0])
                    )
                    .clone()
                    .detach()
                    .to(torch.float32)
                ...
                right_nodes = torch.tensor(
                    right_fragment.subgraphs[0], dtype=torch.int64
                )

                if (
                    len(left_nodes) > MAX_SUBGRAPH_NODES
    ...
    def match_fragments_to_peaks(
        ...
        mz_fragments,
        int_list=None,
        mode_map_override=None,
        tolerance=DEFAULT_PPM,
        match_stats_only: bool = False,
    ):
        self.peak_matches = self.fragmentation_tree.match_peak_list(
            mz_fragments, int_list, tolerance=tolerance
        )
        self.edge_breaks = [
            frag.edges
        ...
        ]  # Flatten the edge breaks
        edge_break_labels = torch.tensor(
            [
                [
                    1.0
                    if (u, v) in self.edge_breaks or (v, u) in self.edge_breaks
                    else 0.0
                    for u, v in self.edges_as_tuples
                ]
            ],
        ...
        self.edge_break_count = torch.zeros(
            size=edge_break_labels.size(), dtype=torch.float32
        )
        self.precursor_count, self.precursor_prob, self.precursor_sqrt_prob = (
            torch.tensor(0.0),
            torch.tensor(0.0),
            torch.tensor(0.0),
        )
        ...
        for edge, values in self.edge_intensities:
            ...
            edge_index = (
                torch.logical_or(
                    self.edges == torch.tensor(edge),
                    self.edges == torch.tensor(edge[::-1]),
                )
                .all(dim=1)
                .nonzero()
                .squeeze()
            ...
            backward_idx = (
                ((torch.tensor(edge[::-1]) == self.edges).sum(dim=1) == 2)
                .nonzero()
                .squeeze()
            )
        ...
        self.compiled_countsALL = torch.cat(
            [
                self.edge_count_matrix.flatten(),
                self.precursor_count.unsqueeze(dim=-1),
                self.precursor_count.unsqueeze(dim=-1),
            ]
        )
        self.compiled_probsALL = (
            2 * self.compiled_countsALL / torch.sum(self.compiled_countsALL)
        )

        # SQRT transformation
        self.compiled_countsSQRT = torch.sqrt(self.compiled_countsALL)
        self.compiled_probsSQRT = (
            2 * self.compiled_countsSQRT / torch.sum(self.compiled_countsSQRT)
        )

        # MASKS
        # self.compiled_validation_mask = torch.cat([self.is_edge_not_in_ring.bool().squeeze(), torch.tensor([True, True], dtype=bool)], dim=-1)
        self.compiled_validation_maskALL = torch.cat(
            [
                torch.repeat_interleave(
                    self.is_edge_not_in_ring.bool().squeeze(), len(mode_map) * 2
                ),
                torch.tensor([True, True], dtype=bool),
            ],
            dim=-1,
        )
    ...
    def get_memory_usage(self):
        memory_usage = {
            attr: sys.getsizeof(value) for attr, value in self.__dict__.items()
        }
        ...
        if with_labels:
            return Data(
                ...
                weight_tensor=torch.full(
                    self.compiled_probsALL.shape, self.loss_weight
                ),
