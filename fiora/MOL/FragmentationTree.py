...
class Fragment:
    def __init__(self, mol, edge=None, isotope_labels=None):
        ...
        self.mz = {
            mode: self.neutral_mass + constants.ADDUCT_WEIGHTS[mode]
...
class FragmentationTree:
    ...
    def build_fragmentation_tree(
        self, mol, edge_indices, depth=2, parent_tree=None, parent_id=None
    ):
        ...
        for i, atom in enumerate(mol.GetAtoms()):
            atom.SetIsotope(
                i
            )  # use isotope information as a proxy for atom_id (such that the information is not lost when carrying out the bond break)

        self.fragmentation_tree.create_node(
            tag=root_fragment.get_tag(),
            parent=parent_id,
            identifier=root_fragment.ID,
            data=root_fragment,
        )
        ...
        for i, j in edge_indices:
            if i > j:
                continue
            _, fragments = self.create_Fragments(
                mol, i, j, original_mol_isotopes=mol_isotopes
            )
            self.edge_map[(i, j)] = {
                frag.break_sides[0]: frag for frag in fragments
            }  # TODO Maybe choose to update edge_map later, this way the same fragment can exist multiple times due to multiple edges leading to the same fragments: Maybe fix. Maybe not.
            ...
            for f in fragments:
                if f is not None:
                    ...
                    if depth == 1:  # anchor
                        ...
                        f.set_ID(self.fragmentation_tree.size())
                        self.fragmentation_tree = self.build_fragmentation_tree(
                            f.MOL,
                            edge_indices,
                            depth=depth - 1,
                            parent_tree=self.fragmentation_tree,
                            parent_id=root_fragment.ID,
                        )

                    listed_fragments.append(f)

        for i, atom in enumerate(mol.GetAtoms()):
            atom.SetIsotope(mol_isotopes[i])  # Reset isotope information
    ...
    def create_Fragments(self, mol, i, j, original_mol_isotopes=None):
        ...
        try:
            ...
            if len(fragment_mols) < 1:  # TODO resolve ring break
                ...
                pass
        return new_mol, [
            Fragment(m, edge=(int(i), int(j)), isotope_labels=original_mol_isotopes)
    ...
    def build_fragmentation_tree_by_single_edge_breaks(
        self, mol, edge_indices, depth=2, parent_tree=None, parent_id=None
    ):
        ...
        self.fragmentation_tree.create_node(
            tag=Chem.Descriptors.ExactMolWt(mol),
            parent=parent_id,
            identifier=ID,
            data=mol,
        )
                ...
                if f is not None:
                    ...
                    if depth == 1:  # anchor
                        self.fragmentation_tree.create_node(
                            ...
                            identifier=self.fragmentation_tree.size(),
                            parent=ID,
                            data=f,
                        )
                        ...
                        G = mol_to_graph(f)
                        A = get_adjacency_matrix(G)
                        edge_indices = (A > 0.001).nonzero(as_tuple=False)
                        edge_indices = edge_indices[
                            edge_indices[:, 0] < edge_indices[:, 1]
                        ].tolist()
                        self.fragmentation_tree = (
                            self.build_fragmentation_tree_by_single_edge_breaks(
                                f,
                                edge_indices,
                                depth=depth - 1,
                                parent_tree=self.fragmentation_tree,
