...
# Fiora GNN Modules
from fiora.GNN.FeatureEmbedding import FeatureEmbedding
...
class FioraModel(torch.nn.Module):
    def __init__(self, model_params: Dict) -> None:
        ...
        self.node_embedding = FeatureEmbedding(
            feature_dict=model_params['node_feature_layout'],
            dim=model_params['embedding_dimension'],
            aggregation_type=model_params['embedding_aggregation'],
        )
        self.edge_embedding = FeatureEmbedding(
            feature_dict=model_params['edge_feature_layout'],
            dim=model_params['embedding_dimension'],
            aggregation_type=model_params['embedding_aggregation'],
        )
        self.GNN_module = GNN(
            ...
            embedding_dim=self.node_embedding.get_embedding_dimension(),
            embedding_aggregation_type=model_params['embedding_aggregation'],
            gnn_type=model_params['gnn_type'],
            layer_norm=model_params['layer_norm'],
            residual_connections=model_params['residual_connections'],
            layer_stacking=model_params['layer_stacking'],
            input_dropout=model_params['input_dropout'],
            latent_dropout=model_params['latent_dropout'],
        )
        self.edge_module = EdgePropertyPredictor(
            edge_feature_dict=model_params['edge_feature_layout'],
            hidden_features=self.GNN_module.get_embedding_dimension(),
            static_features=model_params['static_feature_dimension'],
            out_dimension=model_params['output_dimension'],
            dense_depth=model_params['dense_layers'],
            dense_dim=model_params['dense_dim'],
            embedding_dim=self.edge_embedding.get_embedding_dimension(),
            embedding_aggregation_type=model_params['embedding_aggregation'],
            residual_connections=model_params['residual_connections'],
            subgraph_features=model_params['subgraph_features'],
            pooling_func=model_params['pooling_func'],
            input_dropout=model_params['input_dropout'],
            latent_dropout=model_params['latent_dropout'],
        )
        self.precursor_module = GraphPropertyPredictor(
            hidden_features=self.GNN_module.get_embedding_dimension(),
            static_features=model_params['static_feature_dimension'],
            out_dimension=1,
            dense_depth=model_params['dense_layers'],
        ...
        if model_params['prepare_additional_layers']:
            self.RT_module = GraphPropertyPredictor(
                hidden_features=self.GNN_module.get_embedding_dimension(),
                static_features=model_params['static_rt_feature_dimension'],
                out_dimension=1,
                dense_depth=model_params['dense_layers'],
                dense_dim=model_params['dense_dim'],
                residual_connections=model_params['residual_connections'],
            ...
            self.CCS_module = GraphPropertyPredictor(
                hidden_features=self.GNN_module.get_embedding_dimension(),
                static_features=model_params['static_rt_feature_dimension'],
                out_dimension=1,
                dense_depth=model_params['dense_layers'],
    ...
    def _version_control_model_params(self, model_params: Dict) -> None:
        ...
        if 'prepare_additional_layers' not in model_params:
            model_params['prepare_additional_layers'] = (
                True  # Defaults to True, since old models have RT/CCS modules
            )
        if 'dense_dim' not in model_params:
            model_params['dense_dim'] = (
                None  # None defaults to the number of input features (GNN output dimension)
            )
        if (
    ...
    def set_transform(
        self, transformation: Literal['softmax', 'double_softmax', 'off']
    ):
        self.softmax = torch.nn.Softmax(dim=0)
        if transformation == 'double_softmax':
            self.transform = lambda y: 2.0 * self.softmax(y)  # TODO make torch module
    ...
    def _compile_output(self, edge_values, graph_values, batch) -> torch.tensor:
        output = torch.zeros(
            edge_values.shape[0] * edge_values.shape[1] + graph_values.shape[0] * 2,
        ...
        edge_graph_map = batch['batch'][
            torch.repeat_interleave(batch['edge_index'][0, :], self.edge_dim)
        ]
        for i in range(batch.num_graphs):
            edges = edge_values.flatten()[
                edge_graph_map == i
            ]  # Retrieve edge_values for graph i
            offset = (
                edges.shape[0] + graph_values.shape[1] * 2
            )  # Precursor prediction output is repeated to account for bi-directional edge occurances
            output[batch_ptr : batch_ptr + offset,] = self.transform(
                torch.cat([edges, graph_values[i], graph_values[i]], axis=-1)
            )  # concat and apply softmax
            batch_ptr += offset
            segment_ptr.append(batch_ptr)

        return output, torch.tensor(
            segment_ptr, device=edge_values.device, dtype=torch.long
        )
    ...
    def forward(self, batch, with_RT=False, with_CCS=False):
        ...
        fragment_probs, segment_ptr = self._compile_output(
            edge_values, graph_values, batch
        )
    ...
    def load(cls, PATH: str) -> 'FioraModel':
        ...
        if not isinstance(model, cls):
            raise ValueError(
                f'file {PATH} contains incorrect model class {type(model)}'
            )
    ...
    def load_from_state_dict(cls, PATH: str) -> 'FioraModel':
        ...
        model.load_state_dict(
            torch.load(
                STATE_PATH,
                map_location=torch.serialization.default_restore_location,
                weights_only=True,
            )
        )
