...
class EdgeSingleLabelData(Dataset):
    def __init__(
        ...
        right_tag: str,
        adj_tag: str,
        edge_feature_tag: str,
        static_feature_tag: str,
        label: str,
        ...
        self.A = data.apply(lambda x: getattr(x, adj_tag).to(device)).values
        self.AL = data.apply(
            lambda x: getattr(x, left_tag).to(device)
        ).values  # matrix to list all nodes to the left of an edge
        self.AR = data.apply(
            lambda x: getattr(x, right_tag).to(device)
        ).values  # matrix to list all nodes to the right of an edge
        self.num_nodes = np.array(list(map(lambda x: x.shape[0], self.X)))
        self.y = data.apply(lambda x: getattr(x, label).to(device)).values
        self.num_edges = np.array(list(map(lambda x: x.shape[0], self.y)))
        self.edge_features = data.apply(
            lambda x: getattr(x, edge_feature_tag).to(device)
        ).values
        self.static_features = data.apply(
            lambda x: getattr(x, static_feature_tag).to(device)
        ).values
        self.validation_mask = data.apply(
            lambda x: getattr(x, validation_mask_tag).to(device)
        ).values
    ...
    def __getitem__(self, idx):
        return [
            ...
            self.AL[idx],
            self.AR[idx],
            self.validation_mask[idx],
        ]

    def num_features(self):  # Number of node features
        return self.X[0].shape[1]

    def num_static_features(
        self,
    ):  # Number of additional features concatenated for the edge classification
...
def collate_graph_batch(batch):
    ...
    A = torch.stack(list(map(pad_matrix, A, pad_sizes)))
    y = torch.nn.utils.rnn.pad_sequence(y, batch_first=True, padding_value=0)
    node_mask = torch.zeros((X.shape[0], max_nodes), dtype=torch.bool).to(
        X.get_device()
    )
...
def collate_graph_edge_batch(batch):
    ...
    X = torch.nn.utils.rnn.pad_sequence(X, batch_first=True, padding_value=0)
    edge_features = torch.nn.utils.rnn.pad_sequence(
        edge_features, batch_first=True, padding_value=0
    )
    static_features = torch.nn.utils.rnn.pad_sequence(
        static_features, batch_first=True, padding_value=0
    )

    def pad_matrix(x, y):
        return torch.nn.functional.pad(x, (0, y, 0, y), value=0)

    A = torch.stack(list(map(pad_matrix, A, pad_sizes)))
    y = torch.nn.utils.rnn.pad_sequence(y, batch_first=True, padding_value=0)

    def pad_matrix(x, y, z):
        return torch.nn.functional.pad(
            x, (0, y, 0, z), value=0
        )  # Pad helper matrices to maximum size
