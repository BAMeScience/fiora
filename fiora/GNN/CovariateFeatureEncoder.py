...
class CovariateFeatureEncoder:
        ...
        for feature in self.feature_list:
            if feature in self.categorical_sets.keys():
                ...
                self.one_hot_mapper[feature] = dict(
                    zip(
                        variables,
                        range(self.encoding_dim, len(variables) + self.encoding_dim),
                    )
                )
        ...
        for feature in self.feature_list:
            if feature in self.categorical_sets.keys():
                ...
                if value in self.categorical_sets[feature]:
                    ...
                    feature_matrix[
                        :,
                        self.one_hot_mapper[feature][
                            list(self.categorical_sets[feature])[-1]
                        ]
                        + 1,
                    ] = 1.0
    ...
    def get_element_composition(G):
        ...
        element_composition = torch.zeros(
            len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN), dtype=torch.float32
        )
