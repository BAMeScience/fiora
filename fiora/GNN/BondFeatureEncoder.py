    ...
    def encode(self, G, edges, encoder_type: Literal['one_hot', 'number']):
        ...
        if encoder_type == 'one_hot':
            feature_matrix = torch.zeros(
                len(edges), self.encoding_dim, dtype=torch.float32
            )
            ...
            for i, (u, v) in enumerate(edges):
                ...
                if 'bond_type' in self.feature_list:
                    feature_matrix[i][
                        self.one_hot_mapper['bond_type'][bond.GetBondType().name]
                    ] = 1.0
            ...
            feature_matrix = torch.zeros(
                len(edges), len(self.feature_list), dtype=torch.int
            )
            for i, (u, v) in enumerate(edges):
                bond = G[u][v]['bond']
                    ...
                    if feature == 'bond_type':
                        ...
                        if value in self.sets['bond_type']:
                            ...
                            raise NotImplementedError(
                                'Unknown bond type is not accounted for.'
                            )
                ...
                if feature == 'ring_type_binary':
                    raise NotImplementedError(
                        "Binary feature not implemented with number embedding. Use default 'ring_type' instead."
                    )
