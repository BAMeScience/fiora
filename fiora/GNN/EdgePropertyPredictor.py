...
class EdgePropertyPredictor(torch.nn.Module):
    def __init__(
        ...
        num_subgraph_features = (
            2 * len(ORDERED_ELEMENT_LIST_WITH_HYDROGEN) + hidden_features * 2
    ...
    def concat_node_pairs(self, X, batch):
        ...
        if self.subgraph_features:
            ...
            left_indices = batch.subgraph_idx_left + batch.ptr[
                edge_batch_map
            ].unsqueeze(1)
            right_indices = batch.subgraph_idx_right + batch.ptr[
                edge_batch_map
            ].unsqueeze(1)

            left_batch_vec = torch.arange(num_edges, device=X.device).repeat_interleave(
                left_indices.size(1)
            )
            right_batch_vec = torch.arange(
                num_edges, device=X.device
            ).repeat_interleave(right_indices.size(1))
            ...
            if left_mask.any():
                pooled_left = self.pooling_func(
                    X[left_flat[left_mask]], left_batch_vec[left_mask], size=num_edges
                )
            if right_mask.any():
                pooled_right = self.pooling_func(
                    X[right_flat[right_mask]],
                    right_batch_vec[right_mask],
                    size=num_edges,
                )
            ...
            node_pairs = torch.cat(
                [node_pairs, subgraph_features, edge_elem_comp], dim=1
            )
