...
def cosine_bias_alt(vec, vec_other, cosine_precomputed=None):
    ...
    if cosine_precomputed <= 0.0:
        return 1.0
    bias = np.sqrt(
        np.square(np.dot(vec, vec_other))
        / np.square(np.linalg.norm(vec) * np.linalg.norm(vec_other))
    )
...
def cosine_bias(vec, vec_other, cosine_precomputed=None):
    ...
    bias = np.sqrt(
        np.dot(np.square(vec), np.square(vec_other))
    )  # wrong: bias = np.sqrt(np.square(np.dot(vec, vec_other)) / (np.square(np.linalg.norm(vec)) * np.square(np.linalg.norm(vec_other))))
    return bias / cosine_precomputed
...
def create_mz_map(mz, mz_other, tolerance):
    ...
    for i, mz in enumerate(mz_unique[:-1]):
        if (
            abs(mz_unique[i + 1] - mz) < tolerance
        ):  # TODO this may lead to continuous mz matches that lie >tolerance apart on the edges
