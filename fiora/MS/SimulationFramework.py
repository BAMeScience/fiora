...
class SimulationFramework:
    ...
    def predict_metabolite_property(
        self, metabolite, model: torch.nn.Module | None = None, as_batch: bool = False
    ):
        if not model:
    ...
    def simulate_spectrum(
        ...
        for i, edge in enumerate(metabolite.edges_as_tuples):
            if edge[0] > edge[1]:
                continue  # skip backward directions
    ...
    def simulate_all(
        ...
        with torch.no_grad():
            ...
            for i, data in iterator:
                ...
                df = pd.concat(
                    [df, pd.DataFrame(columns=[x + suffix for x in stats.keys()])]
                )  # Add new empty columns for all statistics
    ...
    def plot_feature_prediction_vectors(
        self, metabolite, label, with_mol=True, transform=None
    ):
        ...
        if with_mol:
            ...
            _, _ = plt.subplots(1, 1, figsize=(12.8, 4.2))

        relevant_edge_index = torch.logical_and(
