...
def annotate_and_plot(
    spectrum, mz_fragments, with_grid: bool = False, ppm_tolerance: int = 100, ax=None
):
    ...
    spectrum = sus.MsmsSpectrum(
        'None', 0, 1, spectrum['peaks']['mz'], spectrum['peaks']['intensity']
    )
...
def plot_spectrum(
    spectrum: Dict,
    second_spectrum: Dict | None = None,
    highlight_matches: bool = False,
    mz_matches: List[int] = [],
    facet_plot=False,
    ppm_tolerance: int = 100,
    charge=0,
    title=None,
    out=None,
    with_grid=False,
    ...
    top_spectrum = sus.MsmsSpectrum(
        'None', 0, charge, spectrum['peaks']['mz'], spectrum['peaks']['intensity']
    )
    if color:
    ...
    if second_spectrum is not None:
        bottom_spectrum = sus.MsmsSpectrum(
            'None',
            0,
            charge,
        ...
        if highlight_matches:
            ...
            x_string = ''.join(
                [f'X[+{mz}]' for mz in sorted(second_spectrum['peaks']['mz'])]
            )
        ...
        if facet_plot:
            sup.facet(
                spec_top=top_spectrum,
                spec_mass_errors=top_spectrum,
                spec_bottom=bottom_spectrum,
...
def plot_vector_spectrum(
    vec1, vec2, ax=None, title=None, y_label='probability', names=None
):
    v1 = pd.DataFrame(
        {'range': names if names else range(len(vec1)), 'prob': vec1, 'group': 'prob'}
    )
    v2 = pd.DataFrame(
        {'range': names if names else range(len(vec2)), 'prob': vec2, 'group': 'pred'}
    )
    V = pd.concat([v1, v2])
